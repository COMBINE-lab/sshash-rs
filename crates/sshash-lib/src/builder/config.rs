...
impl BuildConfiguration {
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        if self.m >= self.k {
            return Err(format!(
                "m must be less than k, got m={}, k={}",
                self.m, self.k
            ));
        }

        // Check that m is less than 32 (so it fits in a u64)
        if self.m > 31 {
            return Err(format!(
                "In the current implementation, m must be < 32, but m={} was passed",
                self.m
            ));
        }
        ...
        if self.lambda < 1.0 || self.lambda > 100.0 {
            return Err(format!(
                "lambda should be in range [1.0, 100.0], got {}",
                self.lambda
            ));
        }
