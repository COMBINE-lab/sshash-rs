...
//!
//! This uses rapidhash with fixed seeds to provide deterministic hashing
//! that is CPU-feature independent (unlike ahash which switches algorithm
//! when AES-NI is available, breaking serialized indices).
...
///
/// Uses rapidhash which produces identical results regardless of target CPU
/// features, unlike ahash which uses different algorithms with/without AES-NI.
#[derive(Clone)]
...
impl DeterministicHasher {
    ...
    /// Hash a u64 value using a seeded RapidHasher
    #[inline]
